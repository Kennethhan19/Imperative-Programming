...
bool valid(const char length[]){
  ...
  if(atol(length)<=0) return false;
  if(atol(length)>INT_MAX) return false;
  long l = strlen(length);
  long i;
    for(i=0; i<l; i++){
      if(isdigit(length[i]) == 0)return false;
    }
  return true;
}
...
int convert(const char length[]){
    if (valid(length) == true) return atol(length);
    else return -1;
}
...
int triangle(long a, long b, long c) {
    ...
    else if ((c>a+b) || (b>a+c) || (a>b+c)) return Impossible;
    else if ((a == b+c) || (b == a+c) || (c == a+b)) return Flat;
    else if (a==b && a==c && b==c)return Equilateral;
    else if (a==b || b==c || c==a )return Isosceles;
    else if ((a*a == b*b + c*c) || (a*a + b*b == c*c) || (a*a + c*c == b*b)) return Right;
    else return Scalene;
}
