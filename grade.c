...
int convert(int n, const char mark[]) {
  if (n == 1){
    if(isdigit(mark[0]) != 0) return atoi(mark);
    else return -1;
  }

  if (n == 2){
    if(isdigit(mark[0]) != 0 && isdigit(mark[0]) !='0' && isdigit(mark[1]) != 0) return atoi(mark);
    else return -1;
  }

  if (n==3){
    if(mark[0] == '1' && mark[1] == '0' && mark[2] == '0') return atoi(mark);
    else return -1;
  }
